...
>
> **👉 Current position (2026-06-18):** Stage A done — app boots in Expo Go (purple/cream + square corners). **Stage B on `authflow`:** **B1·M1 (encrypted `LargeSecureStore`) ✓** and **B1·M2 (email sign-up + OTP-confirmation branch) ✓ app-side, tested.** **Pending Supabase side:** set `enable_confirmations = true` + a token-based confirmation email template (adapt bexforte's `portal-otp.html`) so the OTP confirm flow is live/testable — this is shared with B2 (reset OTP). Then **B1·M3 (sign-in/out hardening)**. Email infra: auth via Supabase Custom SMTP → ZeptoMail (`smtp.zeptomail.com`, US) for prod; local uses Mailpit (:55324). Transactional = Phase 6 edge functions. Open: A1·M2 (CI), A2 (regen types), dev build by B3.
>
