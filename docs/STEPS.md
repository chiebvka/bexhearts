...
>
> **👉 Current position (2026-06-18):** Stage A done — app boots in Expo Go (purple/cream + square corners). **Stage B on `authflow`:** **B1·M1 (encrypted `LargeSecureStore`) ✓** and **B1·M2 (email sign-up + OTP confirmation) ✓** — app-side AND Supabase-side (`enable_confirmations = true` + `confirm-signup.html` template). **Owner must restart Supabase** (`supabase stop && supabase start`) to apply, then test reading the code from Mailpit (:55324). Next: **B1·M3 (sign-in/out hardening)**, then the B2 recovery template reuses this pattern. Email infra: auth via Supabase Custom SMTP → ZeptoMail (`smtp.zeptomail.com`, US) for prod; local uses Mailpit (:55324). Transactional = Phase 6 edge functions. Open: A1·M2 (CI), A2 (regen types), dev build by B3.
>
