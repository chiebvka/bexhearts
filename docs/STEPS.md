...
>
> **👉 Current position (2026-06-18):** Stage A done — app boots in Expo Go (purple/cream + square corners). **Stage B on `authflow`:** **B1·M1 (encrypted `LargeSecureStore`) ✓** and **B1·M2 (email sign-up + OTP confirmation) ✓** — app-side AND Supabase-side (`enable_confirmations = true` + `confirm-signup.html` template). **Owner must restart Supabase** (`supabase stop && supabase start`) to apply, then test reading the code from Mailpit (:55324). **B1·M3 (sign-in/out hardening) ✓** — sign-in routes unconfirmed users to verify (with a fresh code), sign-out cleanup verified; a password show/hide eye toggle was added to the shared `Input` (covers all password fields). Next: **B1·M4 (persistent session)** → M5 (last-used) → M6 (route gating). B2 recovery template reuses the OTP pattern. Email infra: auth via Supabase Custom SMTP → ZeptoMail (`smtp.zeptomail.com`, US) for prod; local uses Mailpit (:55324). Transactional = Phase 6 edge functions. Open: A1·M2 (CI), A2 (regen types), dev build by B3.
>
