...
**Modules:** M1 — pg_cron + edge function (or worker) for: daily devotional reminder, weekly check-in reminder, streak-about-to-break warning, milestone reminders. **M2 — schedule the account-deletion processor** (B4): `cron.schedule('process-account-deletions', '0 3 * * *', $$ select public.process_due_account_deletions(); $$)` so 7-day-grace deletions actually execute (function shipped in `00005`).
**Verify:** scheduled job fires at the configured time in a staging env; an account past its grace deadline is hard-deleted.
