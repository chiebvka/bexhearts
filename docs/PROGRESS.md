...
- [x] **Dashboard** — greeting, both avatars, streak counter, quick actions, today's devotional card. *(2026-06-21: greeting now tied to the device clock via `getGreeting()` in `src/utils/greeting.ts` — morning/afternoon/evening, was hardcoded "Good morning"; tested.)*
...
- [ ] Privacy policy + Terms hosted at real URLs (constants in `src/constants/app.ts` point to placeholders). Suggested: `bexhearts.com/privacy` + `/terms`. **Required by:** Apple App Store Connect (Privacy Policy URL is mandatory), Google Play (Data Safety) + the Google OAuth consent screen *when published to Production* (optional while in Testing).
